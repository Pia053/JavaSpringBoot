...
<section>
    <div class="mt-5">
        <h4 style="font-weight: bold" class="text-center">Cập Nhập Chi Tiết Sản Phẩm</h4>
        <div class="row">
            <div class="col-sm-6 offset-sm-3">
                <form action="/admin/san-pham/update/${sp.id}" method="post" enctype="multipart/form-data">

                    <div class="mb-3">
                        <label class="form-label">Tên Sản Phẩm</label>
                        <input type="text" class="form-control" name="tenSanPham" value="${sp.tenSanPham}" required>
                        <span style="color: red">${ERR.get('ERR_TENSANPHAM')}</span>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Số Lượng Tồn</label>
                        <input type="number" min="1" step="1" class="form-control" name="soLuongTon"
                               value="${sp.soLuongTon}" required>
                        <span style="color: red">${ERR.get('ERR_SOLUONGTON')}</span>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input class="form-control" name="anhSanPham" type="file" id="formFileMultiple">
                    </div>
                    <c:if test="${sp.anhSanPham != null}">
                        <img src="http://localhost:8080/images/${sp.anhSanPham}" height="200px" width="200px"
                             class="img-thumbnail" alt="...">
                    </c:if>

                    <div class="mb-3">
                        <label for="exampleFormControlTextarea1">Mô tả</label>
                        <textarea class="form-control" name="moTa" required id="exampleFormControlTextarea1" rows="3">
                            ${sp.moTa}
                        </textarea>
                        <span style="color: red">${ERR.get('ERR_MOTA')}</span>
                    </div>
                    <button onclick=" return confirm('Bạn có muốn cập nhập') ? alert('Cập nhập thành công'):alert('Cập nhập không thành công') " type="submit" class="btn btn-success">Cập Nhập</button>
                </form>
            </div>
        </div>
    </div>
